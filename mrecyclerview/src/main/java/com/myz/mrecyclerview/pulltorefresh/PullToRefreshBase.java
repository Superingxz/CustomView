package com.myz.mrecyclerview.pulltorefresh;
...
import com.myz.mrecyclerview.pulltorefresh.ILoadingLayout.State;
/**
 * 这个实现了下拉刷新和上拉加载更多的功能
 * 
 * @author Li Hong
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        setOrientation(LinearLayout.VERTICAL);		//设置LinearLayout的控件排列方向
        
        mTouchSlop = ViewConfiguration.get(context).getScaledTouchSlop();	//移动点的保护范围值



        mHeaderLayout = createHeaderLoadingLayout(context, attrs);
        mFooterLayout = createFooterLoadingLayout(context, attrs);
        mRefreshableView = createRefreshableView(context, attrs);	//获得头/脚/以及个人的布局
        
        if (null == mRefreshableView) {
            throw new NullPointerException("Refreshable view can not be null.");
        }
        
        addRefreshableView(context, mRefreshableView);	//添加个人的布局到容器中
        addHeaderAndFooter(context);					//添加头布局和脚布局

        // 得到Header的高度，这个高度需要用这种方式得到，在onLayout方法里面得到的高度始终是0	
        //在oncreate方法执行之后,组件在onResume回调后才完成初始化.
    ...
    public void addHeaderLoadingLayout(LoadingLayout headerlayout){
        mHeaderLayout = headerlayout;
        refreshLoadingViewsSize();
    }

    public void addFooterLoadingLayout(LoadingLayout footerlayout){
    ...
    protected void addHeaderAndFooter(Context context) {
        ...
        addHeader(context, params);
        addFooter(context, params);
    }
     ...
     * @param context context
     */
    protected void addHeader(Context context, ViewGroup.LayoutParams params) {
     ...
     * @param context context
     */
    protected void addFooter(Context context, ViewGroup.LayoutParams params) {
